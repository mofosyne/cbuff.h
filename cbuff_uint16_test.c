...
char *cbuff_uint16_test_capacity_one(void)
{
    uint16_t cbuffer[1] = {0};
    cbuff_uint16_t prefilledBuff = cbuff_uint16_struct_prefill(cbuffer);
    mu_assert("capacity should be 1", cbuff_uint16_capacity(&prefilledBuff) == 1);
    mu_assert("should be empty", cbuff_uint16_is_empty(&prefilledBuff));
    mu_assert("enqueue should succeed", cbuff_uint16_enqueue(&prefilledBuff, 0xAB));
    mu_assert("should be full", cbuff_uint16_is_full(&prefilledBuff));
    mu_assert("count should be 1", cbuff_uint16_count(&prefilledBuff) == 1);
    mu_assert("second enqueue should fail", !cbuff_uint16_enqueue(&prefilledBuff, 0xCD));
    uint16_t d = 0;
    mu_assert("dequeue should succeed", cbuff_uint16_dequeue(&prefilledBuff, &d));
    mu_assert("dequeued value should match", d == 0xAB);
    mu_assert("should be empty after dequeue", cbuff_uint16_is_empty(&prefilledBuff));
    // overwrite on a full capacity-1 buffer: old item replaced, new item dequeued
    cbuff_uint16_enqueue(&prefilledBuff, 0x01);
    cbuff_uint16_enqueue_overwrite(&prefilledBuff, 0x02);
    mu_assert("should still be full after overwrite", cbuff_uint16_is_full(&prefilledBuff));
    mu_assert("overwrite value should replace old item", cbuff_uint16_dequeue(&prefilledBuff, &d) && d == 0x02);
    return 0;
}

char *cbuff_uint16_test_reset(void)
{
    uint16_t cbuffer[BUFF_TEST_SIZE] = {0};
    cbuff_uint16_t prefilledBuff = cbuff_uint16_struct_prefill(cbuffer);
    cbuff_uint16_enqueue(&prefilledBuff, 1);
    cbuff_uint16_enqueue(&prefilledBuff, 2);
    mu_assert("count should be 2 before reset", cbuff_uint16_count(&prefilledBuff) == 2);
    cbuff_uint16_reset(&prefilledBuff);
    mu_assert("should be empty after reset", cbuff_uint16_is_empty(&prefilledBuff));
    mu_assert("count should be 0 after reset", cbuff_uint16_count(&prefilledBuff) == 0);
    mu_assert("enqueue after reset should succeed", cbuff_uint16_enqueue(&prefilledBuff, 0x42));
    uint16_t d = 0;
    mu_assert("dequeue after reset should get new value", cbuff_uint16_dequeue(&prefilledBuff, &d) && d == 0x42);
    return 0;
}

char *cbuff_uint16_test_fill_empty_refill(void)
{
    uint16_t cbuffer[BUFF_TEST_SIZE] = {0};
    cbuff_uint16_t prefilledBuff = cbuff_uint16_struct_prefill(cbuffer);
    for (int i = 0; i < BUFF_TEST_SIZE; i++)
        cbuff_uint16_enqueue(&prefilledBuff, (uint16_t)(i + 1));
    mu_assert("should be full after first fill", cbuff_uint16_is_full(&prefilledBuff));
    for (int i = BUFF_TEST_SIZE; i > 0; i--)
    {
        mu_assert("count should decrement during drain", cbuff_uint16_count(&prefilledBuff) == (size_t)i);
        uint16_t d = 0;
        cbuff_uint16_dequeue(&prefilledBuff, &d);
    }
    mu_assert("should be empty after drain", cbuff_uint16_is_empty(&prefilledBuff));
    // second fill with distinct values to catch stale data bugs
    for (int i = 0; i < BUFF_TEST_SIZE; i++)
        cbuff_uint16_enqueue(&prefilledBuff, (uint16_t)(i + 10));
    for (int i = 0; i < BUFF_TEST_SIZE; i++)
    {
        uint16_t d = 0;
        mu_assert("dequeue on second fill should succeed", cbuff_uint16_dequeue(&prefilledBuff, &d));
        mu_assert("value mismatch on second fill", d == (uint16_t)(i + 10));
    }
    mu_assert("should be empty after second drain", cbuff_uint16_is_empty(&prefilledBuff));
    return 0;
}

char *cbuff_uint16_test_peek_after_dequeue(void)
{
    uint16_t cbuffer[BUFF_TEST_SIZE] = {0};
    cbuff_uint16_t prefilledBuff = cbuff_uint16_struct_prefill(cbuffer);
    ...
    cbuff_uint16_dequeue(&prefilledBuff, &d);
    cbuff_uint16_dequeue(&prefilledBuff, &d);
    mu_assert("count should be 2 after two dequeues", cbuff_uint16_count(&prefilledBuff) == 2);
    mu_assert("peek offset 0 should be 3", cbuff_uint16_peek(&prefilledBuff, &d, 0) && d == 3);
    mu_assert("peek offset 1 should be 4", cbuff_uint16_peek(&prefilledBuff, &d, 1) && d == 4);
    mu_assert("peek offset 2 should fail", !cbuff_uint16_peek(&prefilledBuff, &d, 2));
    // verify peek did not consume items
    mu_assert("count unchanged after peeks", cbuff_uint16_count(&prefilledBuff) == 2);
    // peek on empty
    cbuff_uint16_dequeue(&prefilledBuff, &d);
    cbuff_uint16_dequeue(&prefilledBuff, &d);
    mu_assert("peek on empty should fail", !cbuff_uint16_peek(&prefilledBuff, &d, 0));
    return 0;
}

char *cbuff_uint16_test_overwrite_nonfull(void)
{
    uint16_t cbuffer[BUFF_TEST_SIZE] = {0};
    cbuff_uint16_t prefilledBuff = cbuff_uint16_struct_prefill(cbuffer);
    // enqueue_overwrite on a non-full buffer should behave like regular enqueue
    for (int i = 0; i < BUFF_TEST_SIZE; i++)
        cbuff_uint16_enqueue_overwrite(&prefilledBuff, (uint16_t)(i + 1));
    mu_assert("should be full after overwrite-fill", cbuff_uint16_is_full(&prefilledBuff));
    for (int i = 0; i < BUFF_TEST_SIZE; i++)
    {
        uint16_t d = 0;
        mu_assert("dequeue should succeed", cbuff_uint16_dequeue(&prefilledBuff, &d));
        mu_assert("FIFO order should be preserved", d == (uint16_t)(i + 1));
    }
    return 0;
}
...
static char *all_tests()
{
    ...
    mu_run_test(cbuff_uint16_test_capacity_one);
    mu_run_test(cbuff_uint16_test_reset);
    mu_run_test(cbuff_uint16_test_fill_empty_refill);
    mu_run_test(cbuff_uint16_test_peek_after_dequeue);
    mu_run_test(cbuff_uint16_test_overwrite_nonfull);
    return 0;
}
